...
from src.fabric_utils import assemble_fabric_ontology, generate_fabric_ontology_summary, set_id_seed
...
def __main__():
    ...
    parser.add_argument(
        ...
        required=False,
        default="ttl",
        choices=["ttl", "fabric", "both"],
        help="Output format: 'ttl' for OWL/TTL, 'fabric' for Microsoft Fabric Ontology JSON, 'both' for both formats",
    )
    parser.add_argument(
        "--id-seed",
        type=int,
        required=False,
        default=0,
        help="Seed for Fabric entity ID generation (use different values to avoid ID conflicts between ontologies)",
    )
    parser.add_argument(
        "--categories",
        nargs="+",
        required=False,
        default=None,
        choices=["MasterData", "ReferenceData", "WorkProductComponent", "WorkProduct", "Dataset", "Abstract"],
        help="Filter to specific OSDU categories (e.g., --categories MasterData Dataset)",
    )

    args = parser.parse_args()

    # Set the ID seed for Fabric ontology generation
    if args.format in ["fabric", "both"]:
        set_id_seed(args.id_seed)

    # Load dictionary of schemas via local storage, with updated versions
    schema_dict = load_schemas(args.src)
    
    # Filter schemas by category if specified
    if args.categories:
        filtered_dict = {}
        for key, schema in schema_dict.items():
            include = False
            if "MasterData" in args.categories and "/master-data/" in key:
                include = True
            if "ReferenceData" in args.categories and "/reference-data/" in key:
                include = True
            if "WorkProductComponent" in args.categories and "/work-product-component/" in key:
                include = True
            if "WorkProduct" in args.categories and "/work-product/" in key and "/work-product-component/" not in key:
                include = True
            if "Dataset" in args.categories and "/dataset/" in key:
                include = True
            if "Abstract" in args.categories and "/abstract/" in key:
                include = True
            if include:
                filtered_dict[key] = schema
        print(f"Filtered to {len(filtered_dict)} schemas from categories: {', '.join(args.categories)}")
        schema_dict = filtered_dict
    ...
    output_ttl = args.format in ["ttl", "both"]
    output_fabric = args.format in ["fabric", "both"]

    # Assemble TTL file if requested
    if output_ttl:
        assemble_ttl(
            CLASS_ONTOLOGY_DICT,
            PROP_ONTOLOGY_DICT,
    ...
    # Assemble Fabric Ontology JSON if requested
    if output_fabric:
        # Create display name and filename from ontology name
        display_name = args.ontology_name.upper() + "_Ontology"
        output_filename = args.ontology_name.lower() + "_fabric_ontology"
        ...
        assemble_fabric_ontology(
            ...
            ARRAY_PROPERTIES_DICT,
            dest_filepath=args.dest,
            ontology_name=display_name,
            write_file=(not args.report_metrics),
            output_filename=output_filename,
        )
        
        if args.verbose:
            summary = generate_fabric_ontology_summary(CLASS_ONTOLOGY_DICT, PROP_ONTOLOGY_DICT)
            print(f"\nFabric Ontology Summary:")
